...
llvm::Function *PrototypeAST::codegen() {   // 函数原型
    ...
    llvm::Function *F = llvm::Function::Create(FT, llvm::Function::ExternalLinkage, Name, TheModule.get());  // 创建函数，函数类型为FT，链接方式为ExternalLinkage（表示函数可以被其他Module访问），函数名为Name，函数所在的Module为TheModule
...
llvm::Function *FunctionAST::codegen() {    // 函数定义
    ...
    llvm::BasicBlock *BB = llvm::BasicBlock::Create(*TheContext, "entry", TheFunction);  // 创建一个基本块，名字为entry，所属函数为TheFunction
